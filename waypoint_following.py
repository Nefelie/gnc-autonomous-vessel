...
from LOS_functions import desiredHeadingLOS, reached
...
class LaptopController:
    def __init__(self):
        ...
        self.WPs = [(init_MM_x, init_MM_y), (2, 2), (4, 0), (6, 2)]
        self.previous_WP = self.WPs[0]
        self.current_WP = self.WPs[1]
        self.current_WPidx = 1
        ...
        self.filename = Path('Test6_Aruco_' + self.current_time + '.csv')
        self.fn = Path("Test6_Log_" + self.current_time + '.csv')
    ...
    def loop(self):
        ...
        current_x = self.MMxs[-1]
        current_y = self.MMys[-1]

        print("LOS: ", current_x, current_y, self.previous_WP[0], self.previous_WP[1], self.current_WP[0], self.current_WP[1])

        LOS_radius = 1

        self.setpointheading = desiredHeadingLOS(current_x, current_y, self.previous_WP[0], self.previous_WP[1], self.current_WP[0], self.current_WP[1], LOS_radius)

        WP_acceptance_radius = 0.7
        if reached(current_x, current_y, self.current_WP[0], self.current_WP[1], WP_acceptance_radius) == True:
            self.current_WPidx +=1
            print("=============================reached================================")
            print("=============================reached================================")

            self.previous_WP = self.current_WP
            if self.current_WPidx != len(self.WPs):
                self.current_WP = self.WPs[self.current_WPidx]
            else:
                self.stop_robot = True
                
                

        self.setpoint = self.setpointheading + self.YAW

        print("Heading: ", self.setpointheading, "YAW: ", self.YAW, "Outcome: ", self.setpoint)

        angular_PID = PID(3,0.0,0.0, self.setpoint, limits=(-0.8,0.8))
        ...
        print("RPMS: ", rpm_left, rpm_right)

        self.PIDTune.append(Crequest_angular_z_vel)

        # Motion Model
        MMLspeed = Crequest_linear_x_vel
        MMAspeed = Crequest_angular_z_vel-0.15
        new_heading = self.MMHs[-1] + MMAspeed*0.4*dt
        # new_heading = (new_heading + (self.YAW - self.YAWs[0]))/2
        distance = MMLspeed * dt

        dx = distance*np.cos(new_heading)
        dy = distance*np.sin(new_heading)
        ...
        if current_epoch_s - self.motor_start_time > 30.0:
